...
public class ShowdownHelper extends Helper {
    ...
    public void login(String userName, String password) {
        ...
        By test = By.cssSelector(loginCssSelector);
    ...
    // Challenges the specified user to a battle
    // Using the specified team and format
    public void challengeUser(String format, String team, String user) throws NoSuchChoiceException{
        clickAt(By.cssSelector("button[name='finduser']"));
        waitForElementPresent(By.cssSelector("input.textbox"));
        setValue("input[name='data']", user);
        clickAt(By.cssSelector("button[type='submit']"));
        waitForElementPresent(By.cssSelector("div.userdetails"));
        clickAt(By.cssSelector("button[name='challenge']"));
        waitForElementPresent(By.cssSelector("form.battleform"));
        clickAt(By.cssSelector("form.battleform button[name='format']"));
        waitForElementPresent(By.cssSelector("div.ps-popup"));
        sleep(500);
        By wantedFormatBy = By.xpath(String.format("//button[@name='selectFormat'][text()='%s']", format));
        if (!isElementPresent(wantedFormatBy)) {
        ...
        // Select the desired team
        By teamDropdownBy = By.cssSelector("form.battleform button[name='team']:not([disabled])");
        if (isElementPresent(teamDropdownBy)) {
            if (team != null) {
                clickAt(teamDropdownBy);
                waitForElementPresent(By.cssSelector("div.ps-popup"));
        ...
        clickAt(By.cssSelector("button[name='makeChallenge']"));
    }
    ...
    public void kickInactivePlayer() {
        clickAt(By.cssSelector("button[name='openTimer']"));
        waitForElementPresent(By.cssSelector("button[name='timerOn']"));
        clickAt(By.cssSelector("button[name='timerOn']"));
    ...
    public void createTeam(String importable, String teamName) {
        ...
        // Click New Team
        clickAt(By.cssSelector("button[name='newTop']"));
        waitForElementPresent(By.cssSelector("button[name='import']"));
        clickAt(By.cssSelector("button[name='import']"));
        waitForElementPresent(By.cssSelector("div.teamedit"));
        clear("textarea.textbox");
        setValue("textarea.textbox", importable);
        sleep(500);
        ...
        clickAt(By.cssSelector("button.teambuilderformatselect"));
        sleep(500);
    ...
    public TurnEndStatus waitForNextTurn(int kickAfterSeconds) {
        ...
        while (!isElementPresent(By.cssSelector("div.whatdo")) && !gameOver) {
            ...
            sleep(500);
            waited += 500;
            gameOver = javascript("return app.curRoom.battle.ended;");
        }
        updateBattleLog();
        if (gameOver) {
    ...
    public List<String> getMoves(int slot, boolean getShortNames) {
        ...
        List<String> moves = (List<String>)javascript(
                "var pokeObj = app.curRoom.battle.mySide.pokemon[arguments[0]]; var moves = [];" +
    ...
    public int getSlotForSpecies(String pokemon, String owner) {
        ...
        for (int i = 0; i < 6; ++i) {
            String species = javascript("var p = app.curRoom.battle[arguments[0]].pokemon[arguments[1]]; if (p!=null) return p.species;", side, i);
    ...
    private Object getPokemonAttribute(int slot, String owner, String attribute, boolean json) {
        ...
        if (!json) {
            return javascript("var p=app.curRoom.battle[arguments[0]].pokemon[arguments[1]]; if (p!=null) return p[arguments[2]];", side, slot, attribute);
        }
        else {
            return javascript("var p=app.curRoom.battle[arguments[0]].pokemon[arguments[1]]; if (p!=null) return JSON.stringify(p[arguments[2]]);", side, slot, attribute);
        }
    }
    ...
    public Map<String, Object> getPokemonAttributes(int slot, String owner) {
        ...
        String info = (String)javascript(
                "var p=app.curRoom.battle[arguments[0]].pokemon[arguments[1]];" +
    ...
    public boolean hasVolatile(String pokemon, String owner, String _volatile) {
        ...
        return (Boolean)javascript("var p=app.curRoom.battle[arguments[0]].pokemon[arguments[1]]; if (p!=null) return p.hasVolatile(arguments[2]);", side, slot, _volatile);
    }
    ...
    public Map<String,Integer> getBoosts(String owner) {
        ...
        String jsonBoosts = (String)javascript("return JSON.stringify(app.curRoom.battle[arguments[0]].active[0].boosts);", side);
        try {
    ...
    public String getAbility(String pokemon, String owner, boolean getShortName) {
        return getAbility(getSlotForSpecies(pokemon, owner), owner, getShortName);
    }
    ...
    public String getItem(String pokemon, String owner, boolean getShortName) {
        return getItem(getSlotForSpecies(pokemon, owner), owner, getShortName);
    }
    ...
    public String getCurrentPokemon(String owner, boolean resolveNickname) {
        ...
        String command =
                ("if (app.curRoom.battle.%side.active[0] != null)\n" +
                        "	return app.curRoom.battle.%side.active[0].%field;\n" +
                        "else if (app.curRoom.battle.%side.lastPokemon != null)\n" +
                        "	return app.curRoom.battle.%side.lastPokemon.%field;\n"+
                        "return '';")
    ...
    public List<String> getTeam(String owner) {
        ...
        List<String> team = (List<String>)javascript(
                "var pokes = []; var pokeObjs = app.curRoom.battle[arguments[0]].pokemon; " +
